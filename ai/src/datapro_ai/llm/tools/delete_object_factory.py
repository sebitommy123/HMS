"""delete_object_factory tool — DELETE /object-factories/{id}."""
...
class DeleteObjectFactoryTool(Tool):
    name = "delete_object_factory"

    def definition(self) -> dict[str, Any]:
        return {
            "name": self.name,
            "description": (
                "Delete an object factory. The parent catalog and object "
                "type are NOT affected; only this attachment is removed. "
                "Confirm with the user before calling unprompted."
            ),
            "input_schema": {
                "type": "object",
                "properties": {
