    ...
    with engine.begin() as conn:
        conn.execute(
            text(
                ...
                "(name, logical_name, connector, properties, status, created_at, updated_at) "
                "VALUES (:n1, :n1, 'tpch', :p, 'enabled', NOW(), NOW()), "
                "       (:n2, :n2, :bad_conn, :p, 'enabled', NOW(), NOW())"
            ),
            {
                "n1": "good_one",
                "n2": "bad_one",
    ...
    with engine.begin() as conn:
        conn.execute(
            text(
                "INSERT INTO catalogs "
                "(name, logical_name, connector, properties, status, created_at, updated_at) "
                "VALUES (:n, :n, :c, :p, 'enabled', NOW(), NOW())"
            ),
            [
                {"n": "missing", "c": "tpch", "p": "{}"},
