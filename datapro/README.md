...
- **`trinodb/trino:latest`** on `http://localhost:8080`
...
```bash
docker compose ps
docker inspect -f '{{.State.Health.Status}}' hms-trino
```
...
```bash
docker exec hms-trino trino --execute "SHOW CATALOGS"
# expect: "system"
```

Web UI: `http://localhost:8080` (dev image; user `trino`).
...
- **OOM** — `docker logs hms-trino`; give the VM more RAM.
